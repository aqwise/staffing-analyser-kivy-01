def identify_companies() -> str:
    identified_companies = """
     ...
     2. Любое слово/фрагмент, отделённое пробелом, дефисом или символом «‑», считай возможным названием компании. Если по найденному кандидату нет подтверждённых данных и есть подозрение, что это имя и фамилия сотрудника (например, краткое имя + фамилия, выглядящие так - MSurp, DKast), обязательно укажи это в ответе. 
